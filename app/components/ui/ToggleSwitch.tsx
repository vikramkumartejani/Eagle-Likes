...
const ToggleSwitch: React.FC<ToggleSwitchProps> = ({ checked, onChange }) => {
    return (
        ...
        >
            <span
                className={`absolute left-[3px] top-1/2 -translate-y-1/2 w-[20px] h-[20px] bg-white rounded-full shadow-[0_2px_4px_rgba(0,0,0,0.3)] transition-all duration-300 ease-in-out ${checked ? 'translate-x-[24px]' : 'translate-x-0'
                    }`}
            />
        </button>
    );
